...
const OrganizerInfo_card = () => {
  const {concerts, setConcerts} = useContext(OrganizerContext)
  const {userDeepDetails, setUserDeepDetails, logout} = useContext(AuthContext)
  ...
  if (concerts !== undefined){
    filteredItems = concerts.filter(item => {
 ...
 const [sortStateDirection, setSortStateDirection] = useState({
  ...
  const sortButton = async (command) => {
    if (command === "alphabet") {
      if (sortStateDirection['alphabet'] === 0) {
        ...
        //
        const sortedItems = [...filteredItems].sort((a, b) => b['name'].localeCompare(a['name']))
        setConcerts(sortedItems)
    ...
    if (command === "eventDate") {
      if (sortStateDirection['eventDate'] === 0) {
        const sortedItems = [...filteredItems].sort((a, b) => a['dateValidRange1'].localeCompare(b['dateValidRange1']))
        setConcerts(sortedItems)
        setSortStateDirection({
          ...sortStateDirection,
          alphabet: 0,
        ...
        const sortedItems = [...filteredItems].sort((a, b) => b['dateValidRange1'].localeCompare(a['dateValidRange1']))
        setConcerts(sortedItems)
        setSortStateDirection({
          ...sortStateDirection,
          alphabet: 0,
    ...
    if (command === "revenue") {
      if (sortStateDirection['revenue'] === 0) {
        // ASCENDING FUNCTION HERE
        const sortedItems = [...filteredItems].sort((a, b) => returnTotalValue(b['ticket']) - returnTotalValue(a['ticket']))
        setConcerts(sortedItems)
        ...
        // ASCENDING FUNCTION HERE
        const sortedItems = [...filteredItems].sort((a, b) => returnTotalValue(a['ticket']) - returnTotalValue(b['ticket']))
        setConcerts(sortedItems)
        setSortStateDirection({
          ...sortStateDirection,
          alphabet: 0,
  ...
  if (filteredItems !== undefined && userDeepDetails !== undefined) {
    ...
    <div className='row align-items-start justify-content-between'>
      ...
      <div className='col-4 p-4 m-1 bg-dark text-white rounded'>
        <TextField value={query} onChange={e => setQuery(e.target.value)} id="outlined-basic" label="Search" variant="outlined" color='primary' sx={{width: "100%"}} className='pb-4' focused autoComplete='off'/>
        <h4 className='bolder'>Sort by</h4>
        <ul className='text-decoration-none d-flex justify-content-between mx-2 '>
          <li onClick={() => sortButton("alphabet")} className={stateChecker("alphabet", "text")}>Alphabet {stateChecker("alphabet", "icon")}</li>
          <li onClick={() => sortButton("eventDate")} className={stateChecker("eventDate", "text")}>Event Date{stateChecker("eventDate", "icon")}</li>
          <li onClick={() => sortButton("revenue")} className={stateChecker("revenue", "text")}>Revenue{stateChecker("revenue", "icon")}</li>
      ...
      <div className='col ms-lg-5'>
        {filteredItems.map((eachData) => (
          <Card key={eachData.id} onClick={() => {navigateTo(eachData.name, eachData.id)}} className='p-0 mb-5 user-select-none bg-dark text-light concertCard'>
            <CardActionArea>
              <CardMedia sx={{height: 300}} image={`${mediaBaseUrl}${apiStaticURL}${eachData.bannerImg}`}/>
              <CardContent className='p-4 bg-dark text-light'>
                <div>
                  ...
                  <div className='row justify-content-between'>
                    <h4 className='col text-info'>{`${dayjs(eachData['dateValidRange1']).format('MMM DD')} - ${dayjs(eachData['dateValidRange2']).format('MMM DD')}`}</h4>
